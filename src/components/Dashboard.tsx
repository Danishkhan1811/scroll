            ...
            {files && files?.length !== 0 ? (
                <ul className="mt-8 grid grid-cols-1 gap-6 divide-y divide-zinc-200 md:grid-cols-2 lg:grid-cols-3">
                    {files.sort(
                        ...
                        <li
                            key={file.id}
                            className="cursor-pointer col-span-1 divide-y divide-gray-200 rounded-lg bg-[#272727] shadow transition hover:shadow-lg">
                            <Link href={`/dashboard/${file.id}`}>
                                <div className="pt-6 px-6 flex w-full items-center justify-between space-x-6">
                                    <div className="h-10 w-10 flex-shrink-0 rounded-full bg-gradient-to-r from-[#3A5BE7] to-[#D6277D]"></div>
